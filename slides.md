---
theme: default
title: Réunion de rentrée franchisés RITMODiag
info: Présentation Slidev premium light pour réunion de rentrée franchisés RITMODiag 2026
class: text-left
transition: slide-left
mdc: true
drawings:
  persist: false
favicon: /favicon.svg
fonts:
  sans: Funnel Display
  weights: '300,400,500,600,700'
css: unocss
---

<div class="cover-orn cover-orn--back" aria-hidden="true">
  <img class="orn orn-vert" src="/assets/ornements/vert.svg" alt="" />
  <img class="orn orn-jaune" src="/assets/ornements/jaune.svg" alt="" />
</div>

<div class="cover-head">

<img class="cover-logo" src="/assets/drive/logos/logo-ritmodiag.svg" alt="RITMODiag" />

# Réunion<br><span class="accent">de rentrée franchisés</span>

</div>

<CoverWall />

<div class="cover-orn cover-orn--front" aria-hidden="true">
  <img class="orn orn-rouge" src="/assets/ornements/rouge.svg" alt="" />
</div>

<!--
Notes speaker : ouvrir avec énergie. Insister sur le fait que la journée n'est pas une réunion descendante mais un point de bascule collectif : le réseau existe, maintenant on le structure pour passer à l'étape supérieure.
-->

---
layout: image
---

<img class="photo-cover" src="/assets/reunion-rentree/reunion-rentree-v20260908-13-formation-aziz-1.jpg" />
<div class="photo-overlay"></div>
<div class="photo-content">

<div class="kicker">Énergie réseau</div>

<h1>Se retrouver pour repartir alignés.</h1>

<div class="icon-grid mt-5">
  <div class="icon-item">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M12 3.2l2.7 5.5 6 .9-4.35 4.24 1.03 6L12 17.01l-5.38 2.83 1.03-6L3.3 9.6l6-.9z"/>
    </svg></span>
    <span>Clarifier les priorités de la rentrée</span>
  </div>
  <div class="icon-item">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M4 7h16M4 12h16M4 17h16"/>
      <circle cx="9" cy="7" r="2" class="knob"/>
      <circle cx="15" cy="12" r="2" class="knob"/>
      <circle cx="7.5" cy="17" r="2" class="knob"/>
    </svg></span>
    <span>Aligner les pratiques entre franchisés</span>
  </div>
  <div class="icon-item">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M12 20.3l-1.35-1.24C5.9 14.76 3 12.12 3 8.88A4.38 4.38 0 0 1 7.4 4.5c1.4 0 2.75.65 3.6 1.68l1 1.2 1-1.2A4.65 4.65 0 0 1 16.6 4.5 4.38 4.38 0 0 1 21 8.88c0 3.24-2.9 5.88-7.65 10.18z"/>
    </svg></span>
    <span>Renforcer le sentiment d’appartenance</span>
  </div>
  <div class="icon-item">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M12.00 4.90 L12.94 2.85 L15.85 3.65 L15.61 5.89 L17.02 6.98 L19.13 6.19 L20.63 8.82 L18.88 10.23 L19.10 12.00 L21.15 12.94 L20.35 15.85 L18.11 15.61 L17.02 17.02 L17.81 19.13 L15.18 20.63 L13.77 18.88 L12.00 19.10 L11.06 21.15 L8.15 20.35 L8.39 18.11 L6.98 17.02 L4.87 17.81 L3.37 15.18 L5.12 13.77 L4.90 12.00 L2.85 11.06 L3.65 8.15 L5.89 8.39 L6.98 6.98 L6.19 4.87 L8.82 3.37 L10.23 5.12 Z"/>
      <circle cx="12" cy="12" r="3.1"/>
    </svg></span>
    <span>Définir une méthode commune</span>
  </div>
  <div class="icon-item">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <circle cx="9" cy="8.1" r="3.2"/>
      <path d="M3.3 19.6a5.7 5.7 0 0 1 11.4 0"/>
      <circle cx="16.9" cy="8.8" r="2.3"/>
      <path d="M15.4 14.5a5 5 0 0 1 5.3 5.1"/>
    </svg></span>
    <span>Partager les retours terrain</span>
  </div>
  <div class="icon-item">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M12 2.4c2.8 2.35 4.3 5.7 4.3 9.3 0 1.5-.26 2.95-.75 4.3H8.45A13 13 0 0 1 7.7 11.7c0-3.6 1.5-6.95 4.3-9.3z"/>
      <circle cx="12" cy="9.7" r="1.8"/>
      <path d="M8.7 13.1 5.9 16a2.6 2.6 0 0 0-.8 1.9v2l3.6-2.1"/>
      <path d="M15.3 13.1 18.1 16c.5.5.8 1.2.8 1.9v2l-3.6-2.1"/>
      <path d="M10.5 19.1v2.5M13.5 19.1v2.5"/>
    </svg></span>
    <span>Préparer le développement du réseau</span>
  </div>
</div>

</div>

<!--
Notes speaker : faire le lien avec la dynamique humaine de la journée : anciens, nouveaux, échanges et qualité des relations.
-->

---

<div class="kicker">Programme</div>

## Planning de la journée

<div class="timeline mt-9">

  <div class="step" v-click>
    <div class="time">10:00</div>
    <span class="ico">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <circle cx="9" cy="8.1" r="3.2"/>
        <path d="M3.3 19.6a5.7 5.7 0 0 1 11.4 0"/>
        <circle cx="16.9" cy="8.8" r="2.3"/>
        <path d="M15.4 14.5a5 5 0 0 1 5.3 5.1"/>
      </svg>
    </span>
    <strong>Partage</strong>
    <span class="muted">Rencontre entre franchisés</span>
  </div>

  <div class="step" v-click>
    <div class="time">12:30</div>
    <span class="ico">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <path d="M4 9.5h11.5v5.2A4.3 4.3 0 0 1 11.2 19H8.3A4.3 4.3 0 0 1 4 14.7z"/>
        <path d="M15.5 11h1.7a2.4 2.4 0 0 1 0 4.8h-1.7"/>
        <path d="M7.3 3.4c-.7.9-.7 1.9 0 2.8M11.6 3.4c-.7.9-.7 1.9 0 2.8"/>
      </svg>
    </span>
    <strong>Pause</strong>
    <span class="muted">Repas collectif + photos</span>
  </div>

  <div class="step" v-click>
    <div class="time">14:00</div>
    <span class="ico">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <circle cx="12" cy="12" r="8.4"/>
        <circle cx="12" cy="12" r="4.9"/>
        <circle cx="12" cy="12" r="1.5"/>
      </svg>
    </span>
    <strong>Vision</strong>
    <span class="muted">Objectifs &amp; structuration</span>
  </div>

  <div class="step" v-click>
    <div class="time">18:00</div>
    <span class="ico">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <path d="M6.6 3.5h10.8l-1.15 6.4a4.45 4.45 0 0 1-8.5 0z"/>
        <path d="M12 14.4v5.1M8.6 20.5h6.8"/>
      </svg>
    </span>
    <strong>Convivial</strong>
    <span class="muted">Apéro pétanque</span>
  </div>

</div>

<!--
Notes speaker : ne pas lire le programme. L'utiliser comme promesse de rythme : humain le matin, stratégique l'après-midi, convivial en clôture.
-->

---
layout: image
---

<img class="photo-cover" src="/assets/reunion-rentree/equipe-caen-facade.jpg" style="object-position: 50% 68%" />
<div class="photo-overlay"></div>
<div class="photo-content">

<div class="kicker">Annonce réseau</div>

<h1>Une étape importante pour RITMODiag Caen</h1>

</div>

<!--
Notes speaker : annoncer simplement le passage de relais à Caen, puis expliquer que cela libère de l'énergie côté franchiseur pour structurer et accompagner tout le réseau.
-->

---

<div class="kicker">Questions d'ouverture</div>

## Apprenons<br>à nous connaître davantage

<div class="grid-5 opening-grid mt-8">
  <div class="metric"><div class="num">1</div><div class="label">Présentation et parcours</div></div>
  <div class="metric"><div class="num">2</div><div class="label">Apprendre quelque chose sur vous</div></div>
  <div class="metric"><div class="num">3</div><div class="label">Vision de l’entrepreneuriat</div></div>
  <div class="metric"><div class="num">4</div><div class="label">Point de friction</div></div>
  <div class="metric"><div class="num">5</div><div class="label">Point fort</div></div>
</div>

<div class="photo-strip mt-7">
  <div class="photo-frame"><img src="/assets/reunion-rentree/terrain-echange.jpg" alt="Échange entre franchisés" /></div>
  <div class="photo-frame"><img src="/assets/reunion-rentree/terrain-diagnostic.jpg" alt="Diagnostic sur tablette" /></div>
  <div class="photo-frame"><img src="/assets/reunion-rentree/terrain-intervention.jpg" alt="Intervention terrain" /></div>
  <div class="photo-frame"><img src="/assets/reunion-rentree/terrain-prelevement.jpg" alt="Prélèvement" /></div>
</div>

<!--
Notes speaker : provoquer des échanges concrets et équilibrés : qui êtes-vous, ce qui vous porte, ce qui frotte, ce qui marche. Garder le rythme pour que chacun puisse s'exprimer.
-->

---
layout: cover
---

<div class="cover-orn cover-orn--back" aria-hidden="true">
  <img class="orn orn-vert" src="/assets/ornements/vert.svg" alt="" />
  <img class="orn orn-jaune" src="/assets/ornements/jaune.svg" alt="" />
</div>

<div class="cover-head">

<img class="cover-logo" src="/assets/drive/logos/logo-ritmodiag.svg" alt="RITMODiag" />

# Pause<br><span class="accent">déjeuner</span>

</div>

<CoverWall />

<div class="cover-orn cover-orn--front" aria-hidden="true">
  <img class="orn orn-rouge" src="/assets/ornements/rouge.svg" alt="" />
</div>

<!--
Notes speaker : marquer la coupure. Rappeler l'heure de reprise et inviter les échanges informels : le déjeuner fait partie de la journée, pas une parenthèse.
-->

---
layout: image
---

<img class="photo-cover" src="/assets/reunion-rentree/diagnostic-reseau.jpg" style="object-position: 50% 40%" />
<div class="photo-overlay"></div>
<div class="photo-content">

<div class="kicker">01 — Diagnostic réseau</div>

<h1 style="max-width: none">Où en est RITMODiag&nbsp;?</h1>

</div>

<!--
Notes speaker : reprendre l'énergie de la couverture : ce n'est pas un constat froid, c'est un point de départ commun. Valoriser les acquis avant d'ouvrir les sujets à structurer.
-->

---

<div class="kicker">Forces actuelles</div>

## Une base solide

<div class="accent-grid mt-8">
  <div class="accent-card c-vert" v-click>
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M12 3.2l2.7 5.5 6 .9-4.35 4.24 1.03 6L12 17.01l-5.38 2.83 1.03-6L3.3 9.6l6-.9z"/>
    </svg></span>
    <h3>Marque claire</h3>
    <p>Une identité identifiable et professionnelle.</p>
  </div>
  <div class="accent-card c-jaune" v-click>
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M12 21.2s6.6-5.7 6.6-10.3a6.6 6.6 0 0 0-13.2 0C5.4 15.5 12 21.2 12 21.2z"/><circle cx="12" cy="10.6" r="2.5"/>
    </svg></span>
    <h3>Ancrage local</h3>
    <p>Des franchisés implantés sur leurs secteurs.</p>
  </div>
  <div class="accent-card c-rouge" v-click>
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M14.9 3.6a4.6 4.6 0 0 0-5.2 6.1L3.5 15.9a2 2 0 0 0 2.8 2.8l6.2-6.2a4.6 4.6 0 0 0 6.1-5.2l-2.9 2.9-2.7-.7-.7-2.7z"/>
    </svg></span>
    <h3>Terrain</h3>
    <p>Une vraie connaissance clients &amp; prescripteurs.</p>
  </div>
  <div class="accent-card c-jaune" v-click>
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <circle cx="12" cy="9" r="5.2"/><path d="M8.6 13.4 7.4 21l4.6-2.4 4.6 2.4-1.2-7.6"/>
    </svg></span>
    <h3>Expertise</h3>
    <p>Un métier maîtrisé et reconnu.</p>
  </div>
  <div class="accent-card c-rouge" v-click>
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M3.6 16.6 9.1 11l3.3 3.3 6.4-6.4"/><path d="M14.8 7.9h4.3v4.3"/>
    </svg></span>
    <h3>Entrepreneurs</h3>
    <p>Des profils impliqués dans leur développement.</p>
  </div>
  <div class="accent-card c-vert" v-click>
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <circle cx="6.2" cy="12" r="2.6"/><circle cx="17.6" cy="6.4" r="2.6"/><circle cx="17.6" cy="17.6" r="2.6"/><path d="M8.6 10.9 15.2 7.6M8.6 13.1l6.6 3.3"/>
    </svg></span>
    <h3>Partage</h3>
    <p>Des bonnes pratiques qui circulent déjà.</p>
  </div>
</div>

<!--
Notes speaker : valoriser sans surcharger. Faire sentir que la suite ne part pas de zéro.
-->

---

<div class="split-list">
<div>

<div class="kicker">À renforcer</div>

## Ce qui doit devenir systématique

<div class="quote mt-7">Structurer pour devenir plus performant.</div>

</div>

<ul class="rows one-col">
  <li v-click>Homogénéité des méthodes</li>
  <li v-click>Visibilité locale</li>
  <li v-click>Pilotage par les chiffres</li>
  <li v-click>Animation réseau</li>
  <li v-click>Outils &amp; process</li>
  <li v-click>Accompagnement des lancements</li>
</ul>

</div>

<!--
Notes speaker : expliquer que l'objectif n'est pas d'enlever l'autonomie, mais d'éviter que chacun réinvente seul les mêmes méthodes.
-->

---
layout: image
---

<img class="photo-cover" src="/assets/reunion-rentree/reseau-carte-france.jpg" style="object-position: 50% 45%" />
<div class="photo-overlay"></div>
<div class="photo-content">

<div class="kicker">02 — Le réseau aujourd'hui</div>

<h1>15 agences, une même trajectoire</h1>

</div>

<!--
Notes speaker : transition vers une slide réseau explicite, importante pour les anciens comme les nouveaux.
-->


---

<div class="kicker">Grille réseau</div>

## Une lecture simple des statuts

<div class="grid-3 mt-8">
  <div class="rd-card"><h3>Déjà en activité</h3><div class="agency-list mt-4"><span class="pill active physical">Caen</span><span class="pill active physical">Saint-Lô</span><span class="pill active">Vire / Sud Manche</span><span class="pill active">Le Havre</span><span class="pill active">Angers</span><span class="pill active">Vierzon</span><span class="pill active">Strasbourg</span><span class="pill active">Morlaix</span></div></div>
  <div class="rd-card"><h3>Nouvelles agences</h3><div class="agency-list mt-4"><span class="pill new">Saint-Malo</span><span class="pill new">Deauville / Lisieux</span><span class="pill new">Alençon</span><span class="pill new">Chartres</span><span class="pill new">Évreux</span><span class="pill new physical">Cherbourg</span><span class="pill new">Arras</span></div></div>
  <div class="rd-card"><h3>Agences physiques</h3><div class="agency-list mt-4"><span class="pill physical">Caen</span><span class="pill physical">Saint-Lô</span><span class="pill physical">Cherbourg</span></div><p class="muted mt-5">Des points d’ancrage visibles pour renforcer la proximité locale.</p></div>
</div>

<!--
Notes speaker : utiliser cette grille pour clarifier les statuts sans rallonger la cartographie.
-->

---
layout: image
---

<img class="photo-cover" src="/assets/reunion-rentree/ambition-reseau.jpg" style="object-position: 50% 35%" />
<div class="photo-overlay"></div>
<div class="photo-content">

<div class="kicker">03 — Ambition 2026</div>

<h1>Devenir un réseau de référence</h1>

</div>

<!--
Notes speaker : formuler l'ambition en positif. Ne pas promettre uniquement de la croissance ; promettre de la croissance maîtrisée.
-->


---

<div class="kicker">Zones prioritaires</div>

## Accélérer là où le potentiel est fort

<div class="grid-2 mt-8">
  <div class="rd-card">
    <h3>Objectif grandes métropoles</h3>
    <div class="agency-list mt-5">
      <span class="pill new">Lyon</span><span class="pill new">Paris</span><span class="pill new">Marseille</span><span class="pill new">Nantes</span><span class="pill new">Bordeaux</span>
    </div>
  </div>
  <div class="rd-card">
    <h3>Renforcer les zones limitrophes</h3>
    <div class="agency-list mt-5">
      <span class="pill">Le Mans</span><span class="pill">Tours</span><span class="pill">Rennes</span><span class="pill">Brest</span><span class="pill">Lille</span><span class="pill">Metz</span><span class="pill">Mulhouse</span>
    </div>
  </div>
</div>

<!--
Notes speaker : le développement ne doit pas être perçu comme une dilution. Chaque ouverture doit renforcer la crédibilité de la marque et donc servir les franchisés existants.
-->

---
layout: image
---

<img class="photo-cover" src="/assets/reunion-rentree/piliers-marque.jpg" style="object-position: 50% 66%" />
<div class="photo-overlay"></div>
<div class="photo-content">

<div class="kicker">04 — Les 4 piliers</div>

<h1>Ce qui rend la franchise forte</h1>

</div>

<!--
Notes speaker : introduire les quatre piliers comme une grille simple qui reviendra tout au long de l'année.
-->

---

<div class="kicker">Pilier 1</div>

## Une marque forte

<div class="grid-2 mt-8 items-start">
  <div>
    <div class="quote">Rassurer. Identifier. Donner confiance.</div>
    <p class="subtitle mt-5">Chaque interaction locale nourrit la réputation nationale.</p>
    <div class="proof mt-9">
      <div>
        <div class="num">4,9/5</div>
        <div class="label">145 avis Google</div>
      </div>
      <div>
        <div class="num">5/5</div>
        <div class="label">Pages Jaunes</div>
      </div>
    </div>
  </div>
  <div class="photo-frame shot-top h-80"><img src="/assets/reunion-rentree/reunion-rentree-v20260908-20-capture-de-la-fiche-google-de-caen.png" /></div>
</div>

<!--
Notes speaker : parler attitude, qualité de service, présence locale. La marque est un actif commun.
-->

---

<div class="kicker">Pilier 2</div>

## Des méthodes claires

<div class="grid-2 mt-8 items-center">
  <div class="grid-2 method-cards">
    <div class="accent-card c-vert">
      <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <circle cx="9" cy="8.1" r="3.2"/><path d="M3.3 19.6a5.7 5.7 0 0 1 11.4 0"/><circle cx="16.9" cy="8.8" r="2.3"/><path d="M15.4 14.5a5 5 0 0 1 5.3 5.1"/>
      </svg></span>
      <h3>Prospection</h3>
      <p>Prescripteurs</p>
    </div>
    <div class="accent-card c-jaune">
      <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <path d="M13.4 2.9 5.2 13.6h5.3l-.9 7.5 8.2-10.7h-5.3z"/>
      </svg></span>
      <h3>Réactivité</h3>
      <p>Leads entrants</p>
    </div>
    <div class="accent-card c-rouge">
      <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <path d="M20.3 12a8.3 8.3 0 1 1-2.6-6"/><path d="M18.8 3.4v3.1h-3.1"/><path d="M12 7.6V12l2.9 1.7"/>
      </svg></span>
      <h3>Relance</h3>
      <p>Devis &amp; suivi</p>
    </div>
    <div class="accent-card c-vert">
      <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <path d="M12 3.1 4.9 6v5.4c0 4.3 3 8.3 7.1 9.5 4.1-1.2 7.1-5.2 7.1-9.5V6z"/><path d="m8.9 11.9 2.2 2.2 4-4.4"/>
      </svg></span>
      <h3>Qualité</h3>
      <p>Clients &amp; rapports</p>
    </div>
  </div>
  <div class="photo-frame h-80">
    <img src="/assets/reunion-rentree/methodes-transmission.jpg" style="object-position: 50% 30%" alt="Transmission de la méthode sur le terrain" />
  </div>
</div>

<!--
Notes speaker : les process doivent être simples et applicables. La photo rappelle que la méthode se transmet sur le terrain, pas seulement dans des supports.
-->

---

<div class="kicker">Pilier 3</div>

## Un accompagnement concret

<div class="accent-grid mt-8">
  <div class="accent-card c-vert">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M3.4 8.6h17.2v10.2a1.6 1.6 0 0 1-1.6 1.6H5a1.6 1.6 0 0 1-1.6-1.6z"/><path d="M8.7 8.6V6.1a1.6 1.6 0 0 1 1.6-1.6h3.4a1.6 1.6 0 0 1 1.6 1.6v2.5"/><path d="M3.4 13.2h17.2"/><path d="M10.6 13.2h2.8"/>
    </svg></span>
    <h3>Outils</h3>
    <p>Supports, modèles, tableaux</p>
  </div>
  <div class="accent-card c-jaune">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M3.6 20.1h16.8"/><path d="M4.4 16.2 9 11.4l3.2 3 5.4-6.6"/><circle cx="12.2" cy="14.4" r="1.5"/>
    </svg></span>
    <h3>Suivi</h3>
    <p>Points réguliers, priorités</p>
  </div>
  <div class="accent-card c-rouge">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M12 6.8C10.4 5.4 8.4 4.7 6 4.7H3.4v13.1H6c2.4 0 4.4.7 6 2.1 1.6-1.4 3.6-2.1 6-2.1h2.6V4.7H18c-2.4 0-4.4.7-6 2.1z"/><path d="M12 6.8v13.1"/>
    </svg></span>
    <h3>Formation</h3>
    <p>Pratique, terrain, action</p>
  </div>
</div>

<!--
Notes speaker : l'accompagnement doit être utile, pratique et directement applicable. Éviter les promesses trop vagues : parler de cadre et d'animation.
-->

---

<div class="kicker">Pilier 4</div>

## Une dynamique collective

<div class="grid-2 mt-8 items-center">
  <div>
    <div class="quote">Les bonnes pratiques d’un franchisé doivent faire progresser tout le réseau.</div>
    <div class="tag-row mt-8">
      <span class="pill c-vert">retours terrain</span>
      <span class="pill c-jaune">entraide</span>
      <span class="pill c-rouge">standards</span>
      <span class="tag-break" aria-hidden="true"></span>
      <span class="pill c-vert">partage</span>
      <span class="pill c-jaune">exigence</span>
    </div>
  </div>
  <div class="photo-frame h-80">
    <img src="/assets/reunion-rentree/dynamique-collective.jpg" style="object-position: 50% 40%" alt="Échange entre collègues devant un écran" />
  </div>
</div>

<!--
Notes speaker : insister sur la responsabilité collective. Le réseau vit par les échanges, pas uniquement par le franchiseur.
-->

---

<div class="kicker">Rôle attendu</div>

## Ce que chacun porte

<ul class="rows mt-8">
  <li v-click>Développer son secteur</li>
  <li v-click>Participer à la vie du réseau</li>
  <li v-click>Représenter la marque localement</li>
  <li v-click>Remonter les informations terrain</li>
  <li v-click>Appliquer les méthodes commerciales</li>
  <li v-click>Être force de proposition</li>
  <li v-click>Garantir une qualité homogène</li>
  <li v-click>Respecter les standards communs</li>
</ul>

<!--
Notes speaker : ne pas transformer la slide en liste de contraintes ; parler d'un cadre qui protège la marque et aide chacun à performer.
-->

---

<div class="kicker">Valeurs</div>

## Ce qui doit se voir dans nos actes

<div class="accent-grid cols-5 mt-8">
  <div class="accent-card c-vert">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M3.4 14.9 14.9 3.4l5.7 5.7L9.1 20.6z"/><path d="m7.3 11 1.9 1.9M10.2 8.1l1.9 1.9M13.1 5.2l1.9 1.9"/>
    </svg></span>
    <h3>Rigueur</h3>
  </div>
  <div class="accent-card c-jaune">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M12 21.2s6.6-5.7 6.6-10.3a6.6 6.6 0 0 0-13.2 0C5.4 15.5 12 21.2 12 21.2z"/><circle cx="12" cy="10.6" r="2.5"/>
    </svg></span>
    <h3>Proximité</h3>
  </div>
  <div class="accent-card c-rouge">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M2.4 12S6 5.6 12 5.6 21.6 12 21.6 12 18 18.4 12 18.4 2.4 12 2.4 12z"/><circle cx="12" cy="12" r="3"/>
    </svg></span>
    <h3>Transparence</h3>
  </div>
  <div class="accent-card c-vert">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <circle cx="6.2" cy="12" r="2.6"/><circle cx="17.6" cy="6.4" r="2.6"/><circle cx="17.6" cy="17.6" r="2.6"/><path d="M8.6 10.9 15.2 7.6M8.6 13.1l6.6 3.3"/>
    </svg></span>
    <h3>Esprit réseau</h3>
  </div>
  <div class="accent-card c-jaune">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M3.6 17.4a9 9 0 1 1 16.8 0"/><path d="m12 13.6 4.1-4.3"/><circle cx="12" cy="15.1" r="1.5"/>
    </svg></span>
    <h3>Performance</h3>
  </div>
</div>

<div class="subtitle mt-8">Les valeurs ne se déclarent pas : elles se remarquent dans le service.</div>

<!--
Notes speaker : relier aux clients, aux délais, aux rapports, aux échanges entre franchisés.
-->

---

<div class="kicker">Tendances terrain</div>

## Les attentes montent

<div class="accent-grid mt-8">
  <div class="accent-card c-vert">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <circle cx="12" cy="13.4" r="7.6"/><path d="M12 9.4v4l2.5 1.5"/><path d="M9.6 2.9h4.8"/><path d="m18.3 6.2 1.5-1.5"/>
    </svg></span>
    <h3>Vite</h3>
    <p>Réactivité au premier contact</p>
  </div>
  <div class="accent-card c-jaune">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M2.4 12S6 5.6 12 5.6 21.6 12 21.6 12 18 18.4 12 18.4 2.4 12 2.4 12z"/><circle cx="12" cy="12" r="3"/>
    </svg></span>
    <h3>Clair</h3>
    <p>Prix, pédagogie, discours</p>
  </div>
  <div class="accent-card c-rouge">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M12 3.1 4.9 6v5.4c0 4.3 3 8.3 7.1 9.5 4.1-1.2 7.1-5.2 7.1-9.5V6z"/><path d="m8.9 11.9 2.2 2.2 4-4.4"/>
    </svg></span>
    <h3>Fiable</h3>
    <p>Délais, livrables, suivi</p>
  </div>
</div>

<!--
Notes speaker : les prescripteurs veulent des partenaires sérieux, disponibles et réguliers. Les clients sont plus informés et plus exigeants.
-->

---

<div class="kicker">Méthode commerciale</div>

## 6 leviers simples

<div class="accent-grid mt-8">
  <div class="accent-card c-vert">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <circle cx="9" cy="8.1" r="3.2"/><path d="M3.3 19.6a5.7 5.7 0 0 1 11.4 0"/><circle cx="16.9" cy="8.8" r="2.3"/><path d="M15.4 14.5a5 5 0 0 1 5.3 5.1"/>
    </svg></span>
    <h3>Prescripteurs</h3>
    <p>Agences, notaires, syndics, gestionnaires, courtiers, bâtiment</p>
  </div>
  <div class="accent-card c-jaune">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <circle cx="10.7" cy="10.7" r="7"/><path d="M3.9 10.7h13.6M10.7 3.7c1.8 2 2.8 4.4 2.8 7s-1 5-2.8 7c-1.8-2-2.8-4.4-2.8-7s1-5 2.8-7z"/><path d="m15.8 15.8 4.6 4.6"/>
    </svg></span>
    <h3>Marketing</h3>
    <p>Site, SEO local/national, génération de leads</p>
  </div>
  <div class="accent-card c-rouge">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M15.6 3.4h5v5"/><path d="m20.6 3.4-6.2 6.2"/><path d="M10.3 4.4 12 8.1l-2 1.8a11 11 0 0 0 4.1 4.1l1.8-2 3.7 1.7v3.3a2 2 0 0 1-2.2 2A16.4 16.4 0 0 1 3.1 8.6a2 2 0 0 1 2-2.2h3.3z"/>
    </svg></span>
    <h3>Suivi leads</h3>
    <p>Rappel rapide, relance, transformation</p>
  </div>
  <div class="accent-card c-jaune">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M3.4 7.6h17.2l-1.7 10.2a2 2 0 0 1-2 1.6H7.1a2 2 0 0 1-2-1.6z"/><path d="M8.6 7.6 12 3.4l3.4 4.2"/><path d="M9.7 11.4v4.2M14.3 11.4v4.2"/>
    </svg></span>
    <h3>Panier moyen</h3>
    <p>Prestations complémentaires &amp; expertise</p>
  </div>
  <div class="accent-card c-rouge">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M12 20.3l-1.35-1.24C5.9 14.76 3 12.12 3 8.88A4.38 4.38 0 0 1 7.4 4.5c1.4 0 2.75.65 3.6 1.68l1 1.2 1-1.2A4.65 4.65 0 0 1 16.6 4.5 4.38 4.38 0 0 1 21 8.88c0 3.24-2.9 5.88-7.65 10.18z"/>
    </svg></span>
    <h3>Fidélisation</h3>
    <p>Relation, avis Google, lien prescripteur</p>
  </div>
  <div class="accent-card c-vert">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <rect x="3.4" y="5.4" width="17.2" height="15.2" rx="2"/><path d="M3.4 10.2h17.2M8.2 3.4v4M15.8 3.4v4"/><path d="M7.6 14.1h2.2M12.1 14.1h2.2M16.6 14.1h.9M7.6 17.4h2.2M12.1 17.4h2.2"/>
    </svg></span>
    <h3>Rythme</h3>
    <p>Une méthode appliquée chaque jour</p>
  </div>
</div>

<!--
Notes speaker : détailler oralement. Garder la slide en macro-leviers pour éviter de noyer l'auditoire.
-->

---

<div class="kicker">Visibilité locale</div>

## Cohérence nationale, impact local

<ul class="rows mt-8">
  <li v-click>Google Business Profile à jour</li>
  <li v-click>Communication locale</li>
  <li v-click>Avis clients collectés régulièrement</li>
  <li v-click>Réseau de prescripteurs entretenu</li>
  <li v-click>Supports commerciaux homogènes</li>
  <li v-click>Discours simple et professionnel</li>
</ul>

<!--
Notes speaker : le client doit comprendre vite qui nous sommes, ce que nous faisons et pourquoi il peut nous faire confiance.
-->

---
layout: fact
---

<div class="kicker">Phrase à marteler</div>

# On ne pilote pas une agence uniquement au ressenti.

<div class="subtitle mx-auto mt-8">Les chiffres servent à comprendre, corriger et progresser.</div>

<!--
Notes speaker : installer la logique de pilotage sans culpabiliser. Un indicateur est un outil d'aide, pas une sanction.
-->

---

<div class="kicker">Indicateurs</div>

## Les bons chiffres, simplement

<div class="accent-grid cols-4 mt-8">
  <div class="accent-card c-vert">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M3.6 4.4h16.8l-6.5 7.7v6.7l-3.8 2.2v-8.9z"/>
    </svg></span>
    <h3>Leads</h3>
    <p>Volume &amp; origine</p>
  </div>
  <div class="accent-card c-jaune">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M10.3 4.4 12 8.1l-2 1.8a11 11 0 0 0 4.1 4.1l1.8-2 3.7 1.7v3.3a2 2 0 0 1-2.2 2A16.4 16.4 0 0 1 3.1 8.6a2 2 0 0 1 2-2.2h3.3z"/><path d="M15.4 3.4v2.7h2.7"/>
    </svg></span>
    <h3>Rappel</h3>
    <p>Délai moyen</p>
  </div>
  <div class="accent-card c-rouge">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M13.6 3.4H6.9a1.9 1.9 0 0 0-1.9 1.9v13.4a1.9 1.9 0 0 0 1.9 1.9h10.2a1.9 1.9 0 0 0 1.9-1.9V8.6z"/><path d="M13.6 3.4v5.2h5.4"/><path d="M8.4 13h7.2M8.4 16.6h5"/>
    </svg></span>
    <h3>Devis</h3>
    <p>Envoyés &amp; relancés</p>
  </div>
  <div class="accent-card c-vert">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M3.6 8.2h13.1l-3-3M20.4 15.8H7.3l3 3"/>
    </svg></span>
    <h3>Transformation</h3>
    <p>Taux de conversion</p>
  </div>
  <div class="accent-card c-jaune">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <circle cx="12" cy="12" r="8.6"/><path d="M15.4 8.6a4.3 4.3 0 0 0-6.9 3.4 4.3 4.3 0 0 0 6.9 3.4"/><path d="M7.6 10.8h5.2M7.6 13.2h5.2"/>
    </svg></span>
    <h3>CA</h3>
    <p>Mensuel</p>
  </div>
  <div class="accent-card c-rouge">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M3.4 7.6h17.2l-1.7 10.2a2 2 0 0 1-2 1.6H7.1a2 2 0 0 1-2-1.6z"/><path d="M8.6 7.6 12 3.4l3.4 4.2"/><path d="M9.7 11.4v4.2M14.3 11.4v4.2"/>
    </svg></span>
    <h3>Panier</h3>
    <p>Moyen</p>
  </div>
  <div class="accent-card c-vert">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <circle cx="9" cy="8.1" r="3.2"/><path d="M3.3 19.6a5.7 5.7 0 0 1 11.4 0"/><circle cx="16.9" cy="8.8" r="2.3"/><path d="M15.4 14.5a5 5 0 0 1 5.3 5.1"/>
    </svg></span>
    <h3>Prescripteurs</h3>
    <p>Actifs</p>
  </div>
  <div class="accent-card c-jaune">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M12 3.2l2.7 5.5 6 .9-4.35 4.24 1.03 6L12 17.01l-5.38 2.83 1.03-6L3.3 9.6l6-.9z"/>
    </svg></span>
    <h3>Avis</h3>
    <p>Clients Google</p>
  </div>
</div>

<!--
Notes speaker : ne pas tout détailler. Proposer de choisir quelques indicateurs clés à suivre régulièrement pour chaque agence.
-->

---

<div class="kicker">Trois axes</div>

## Consolider • Ouvrir • Renforcer

<div class="accent-grid mt-8">
  <div class="accent-card c-vert">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="m12 3.4 8.4 4.3L12 12 3.6 7.7z"/><path d="m3.6 12 8.4 4.3 8.4-4.3"/><path d="m3.6 16.3 8.4 4.3 8.4-4.3"/>
    </svg></span>
    <h3>Consolider l’existant</h3>
    <p>Aider à performer, lever les blocages, harmoniser</p>
  </div>
  <div class="accent-card c-jaune">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M12 21.2s6.6-5.7 6.6-10.3a6.6 6.6 0 0 0-13.2 0C5.4 15.5 12 21.2 12 21.2z"/><circle cx="12" cy="10.6" r="2.5"/>
    </svg></span>
    <h3>Ouvrir les zones</h3>
    <p>Secteurs à potentiel, profils sélectionnés, lancements accompagnés</p>
  </div>
  <div class="accent-card c-rouge">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M3.6 10.1v3.8a1.6 1.6 0 0 0 1.6 1.6h2.2l8.4 4.4V4.1L7.4 8.5H5.2a1.6 1.6 0 0 0-1.6 1.6z"/><path d="M19.4 9.3a3.6 3.6 0 0 1 0 5.4"/><path d="M7.4 15.5v3.4a1.4 1.4 0 0 0 2.7.4"/>
    </svg></span>
    <h3>Renforcer la marque</h3>
    <p>Visibilité et homogénéité au service de tous</p>
  </div>
</div>

<!--
Notes speaker : expliciter la logique réseau : chaque nouvelle implantation doit augmenter la crédibilité nationale et donc soutenir le local.
-->

---
layout: section
---

<div class="section-tag">05 — Ce que le franchiseur met en place</div>

# Plus de cadre, plus d’outils, plus de rythme

<div class="subtitle mt-4">Thomas, Tristan et Océane dédiés au développement de la franchise.</div>

<div class="team-portraits mt-7">
  <div class="person-card">
    <div class="person-photo">
      <img src="/assets/reunion-rentree/reunion-rentree-v20260908-09-thomas-fond-noir.png" alt="Portrait de Thomas sur fond noir" />
    </div>
    <div class="person-name">Thomas</div>
    <div class="person-role">Animation & suivi réseau</div>
  </div>
  <div class="person-card">
    <div class="person-photo">
      <img src="/assets/reunion-rentree/reunion-rentree-v20260908-01-tristan-fond-noir-9.png" alt="Portrait de Tristan sur fond noir" />
    </div>
    <div class="person-name">Tristan</div>
    <div class="person-role">Cadre & développement franchise</div>
  </div>
  <div class="person-card">
    <div class="person-photo">
      <img src="/assets/reunion-rentree/reunion-rentree-v20260908-05-oce-fond-noir.png" alt="Portrait d’Océane sur fond noir" />
    </div>
    <div class="person-name">Océane</div>
    <div class="person-role">Outils & accompagnement terrain</div>
  </div>
</div>

<!--
Notes speaker : reformuler l'image interne maladroite du PDF en message premium. L'idée : implication totale et disponibilité.
-->


---

<div class="kicker">Votre rôle</div>

## Ce que le réseau attend de chaque agence

<ul class="rows mt-8">
  <li v-click>Participation active à la vie du réseau</li>
  <li v-click>Réactivité et professionnalisme avec vos clients</li>
  <li v-click>Application des méthodes communes</li>
  <li v-click>Retours terrain réguliers</li>
  <li v-click>Qualité de service constante avec vos clients</li>
  <li v-click>Volonté de progresser</li>
</ul>

<div class="quote mt-8">Faire vivre la promesse RITMODiag, chaque jour, sur le terrain.</div>

<!--
Notes speaker : insister sur le rôle actif des franchisés : le réseau n'existe pas uniquement par le franchiseur. Chaque agence porte la qualité, la méthode et les retours terrain qui permettent de progresser collectivement.
-->

---

<div class="kicker">Transmission terrain</div>

## Des formats vivants et applicables

<div class="grid-2 mt-8 items-center">
  <div class="photo-frame h-80"><img src="/assets/reunion-rentree/reunion-rentree-v20260908-14-formation-aziz-2.jpg" /></div>
  <ul class="rows one-col">
    <li>Terrain réel</li>
    <li>Gestes métier</li>
    <li>Échanges entre franchisés</li>
    <li>Transmission de méthode</li>
  </ul>
</div>

<!--
Notes speaker : utiliser cette slide comme respiration visuelle pour rendre l'immersion concrète avant de revenir à la méthode.
-->

---

<div class="kicker">Immersion Formation</div>

## Du savoir-faire au réflexe réseau

<div class="accent-grid mt-8">
  <div class="accent-card c-vert">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M2.4 12S6 5.6 12 5.6 21.6 12 21.6 12 18 18.4 12 18.4 2.4 12 2.4 12z"/><circle cx="12" cy="12" r="3"/>
    </svg></span>
    <h3>Voir</h3>
    <p>Observer les bonnes pratiques</p>
  </div>
  <div class="accent-card c-jaune">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <path d="M14.9 3.6a4.6 4.6 0 0 0-5.2 6.1L3.5 15.9a2 2 0 0 0 2.8 2.8l6.2-6.2a4.6 4.6 0 0 0 6.1-5.2l-2.9 2.9-2.7-.7-.7-2.7z"/>
    </svg></span>
    <h3>Faire</h3>
    <p>Appliquer en situation</p>
  </div>
  <div class="accent-card c-rouge">
    <span class="ico"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      <circle cx="6.2" cy="12" r="2.6"/><circle cx="17.6" cy="6.4" r="2.6"/><circle cx="17.6" cy="17.6" r="2.6"/><path d="M8.6 10.9 15.2 7.6M8.6 13.1l6.6 3.3"/>
    </svg></span>
    <h3>Partager</h3>
    <p>Capitaliser pour tout le réseau</p>
  </div>
</div>

<!--
Notes speaker : positionner la formation comme outil de duplication de qualité, pas comme simple obligation.
-->

---

<div class="kicker">Discussion franchisés</div>

## Questions fortes

<ul class="rows mt-8">
  <li>Qu’est-ce qui fonctionne vraiment aujourd’hui&nbsp;?</li>
  <li>Quelles actions commerciales marchent le mieux&nbsp;?</li>
  <li>Qu’est-ce qui bloque le plus souvent&nbsp;?</li>
  <li>Quelles difficultés avec les prescripteurs&nbsp;?</li>
  <li>Quels outils vous manquent&nbsp;?</li>
  <li>Quelle bonne pratique partager au réseau&nbsp;?</li>
</ul>

<!--
Notes speaker : faciliter l'échange. Noter les thèmes qui reviennent. Prioriser le concret.
-->

---

<div class="kicker">Checklist action</div>

## Ce qu’on enclenche dès la rentrée

<div class="grid-2 mt-8">
  <div class="checklist">
    <div class="check" v-click>Clarifier les priorités commerciales de chaque agence</div>
    <div class="check" v-click>Suivre quelques indicateurs simples, chaque mois</div>
    <div class="check" v-click>Renforcer la visibilité locale et les avis clients</div>
  </div>
  <div class="checklist">
    <div class="check" v-click>Partager les bonnes pratiques qui fonctionnent</div>
    <div class="check" v-click>Accompagner les lancements avec méthode</div>
    <div class="check" v-click>Faire vivre une marque commune, homogène et exigeante</div>
  </div>
</div>

<!--
Notes speaker : transformer les échanges en actions. Faire ressortir les engagements ou besoins prioritaires.
-->

---
layout: fact
---

# Notre réussite dépendra de notre capacité à <span class="accent">avancer ensemble.</span>

<div class="subtitle mx-auto mt-8">Avec méthode, exigence et esprit réseau.</div>

<PhotoBand />

<!--
Notes speaker : synthèse finale. Reprendre la vision, remercier, ouvrir vers la suite et le moment convivial.
-->

---
layout: cover
---

<div class="cover-head cover-head--sm">

<img class="cover-logo" src="/assets/drive/logos/logo-ritmodiag.svg" alt="RITMODiag" />

# Merci pour votre présence<br><span class="accent">et votre engagement</span>

<div class="petanque mt-6">

<div class="petanque-line">Maintenant, tu tires ou tu pointes&nbsp;?</div>

<p class="subtitle mt-2">18h00 — Apéro pétanque<br>Place des Oliviers, Les Carandes<br>Parc Évasion, 14120 Mondeville</p>

</div>

</div>

<CoverWall />

<!--
Notes speaker : terminer simplement. Remercier les anciens, accueillir les nouveaux, rappeler l'ambition collective.
-->
